...
const EditTestimonail = () => {
    ...
    const getAllTestimonial = async () => {
        try {
            ...
            const response = await axios.get(`${endpoints.TestimonialsApi}/${id}`, {
            ...
            setformData({
                userImage: testimonial.userImage || null,
                fullName: testimonial.fullName || "",
                designation: testimonial.designation || "",
                propertyImage: testimonial.propertyImage || null,
                clientFeedback: testimonial.clientFeedback || ""
            });

            if (testimonial.userImage) {
                setUserImagePreview(testimonial.userImage);
            }
            if (testimonial.propertyImage) {
                setPropertyImagePreview(testimonial.propertyImage);
            }
    ...
    const UpdateTestimonial = async () => {
        ...
        try {
            ...
            if (formData.propertyImage instanceof File) {
                submitData.append('propertyImage', formData.propertyImage);
            }

            submitData.append('clientFeedback', formData.clientFeedback);

            const response = await axios.patch(`${endpoints.TestimonialsApi}/${id}`, submitData, {
                headers: {
            ...
            toast.success(response.data.message || "Testimonial updated successfully");
            handleNavigate();
        } catch (error) {
            toast.error(error.response?.data?.message || "An error occurred");
        } finally {
        ...
        <>
            <Box sx={{ display: 'flex', flexFlow: "row", justifyContent: 'space-between', width: '100%', }}>
                <Typography
                    ...
                    Dashboard / Testimonial /<span style={{ color: "#4079ED" }}> Edit Testimonial</span>
                </Typography>
            </Box >
                            ...
                            <Grid container >
                                <Grid size={{ xs: 12, md: 12 }}>
                                    <Button
                                        ...
                                        disabled={loading}
                                    >
                                        {loading ? <CircularProgress size={24} sx={{ color: "#FFFFFF" }} /> : "Update Testimonial"}
                                    </Button>
                                </Grid>
                            </Grid>
                        </Box>
